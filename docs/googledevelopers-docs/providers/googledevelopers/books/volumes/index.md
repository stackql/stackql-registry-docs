...
<tr><td><b>Id</b></td><td><code>googledevelopers.books.volumes</code></td></tr>
</tbody></table>
...
|:-----|:--------------|:----------------|:------------|
| `bookshelves_volumes_list` | `SELECT` | `shelf, userId` | Retrieves volumes in a specific bookshelf for the specified user. |
| `get` | `SELECT` | `volumeId` | Gets volume information for a single volume. |
| `list` | `SELECT` | `q` | Performs a book search. |
| `mylibrary_bookshelves_volumes_list` | `SELECT` | `shelf` | Gets volume information for volumes on a bookshelf. |
