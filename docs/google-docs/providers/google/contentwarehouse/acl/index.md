...
Creates, updates, deletes or gets an <code>acl</code> resource or lists <code>acl</code> in a region
...
|:-----|:--------------|:----------------|:------------|
| <CopyableCode code="fetch_acl" /> | `SELECT` | <CopyableCode code="projectsId" /> | Gets the access control policy for a resource. Returns NOT_FOUND error if the resource does not exist. Returns an empty policy if the resource exists but does not have a policy set. |
