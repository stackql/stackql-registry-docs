...
|:-----|:--------------|:----------------|:------------|
| `sql_connect_get` | `SELECT` | `instance, project` | Retrieves connect settings about a Cloud SQL instance. |
| `sql_connect_generateEphemeral` | `EXEC` | `instance, project` | Generates a short-lived X509 certificate containing the provided public key and signed by a private key specific to the target instance. Users may use the certificate to authenticate as themselves when connecting to the database. |
