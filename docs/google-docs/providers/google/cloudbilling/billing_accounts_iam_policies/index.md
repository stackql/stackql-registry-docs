...
<tr><td><b>Id</b></td><td><CopyableCode code="cloudbilling.billing_accounts_iam_policies" /></td></tr>
</tbody></table>
...
| <CopyableCode code="_get_iam_policy" /> | `EXEC` | <CopyableCode code="billingAccountsId" /> | Gets the access control policy for a billing account. The caller must have the `billing.accounts.getIamPolicy` permission on the account, which is often given to billing account [viewers](https://cloud.google.com/billing/docs/how-to/billing-access). |
