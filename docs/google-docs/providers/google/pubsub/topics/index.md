...
| <CopyableCode code="projects_topics_create" /> | `EXEC` | <CopyableCode code="projectsId, topicsId" /> | Creates the given topic with the given name. See the [resource name rules] (https://cloud.google.com/pubsub/docs/admin#resource_names). |
| <CopyableCode code="projects_topics_patch" /> | `EXEC` | <CopyableCode code="projectsId, topicsId" /> | Updates an existing topic. Note that certain properties of a topic are not modifiable. |
