...
| `transferJobs_delete` | `DELETE` | `projectId, transferJobsId` | Deletes a transfer job. Deleting a transfer job sets its status to DELETED. |
...
| `transferJobs_run` | `EXEC` | `transferJobsId` | Starts a new operation for the specified transfer job. A `TransferJob` has a maximum of one active `TransferOperation`. If this method is called while a `TransferOperation` is active, an error is returned. |
