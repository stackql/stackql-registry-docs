...
Creates, updates, deletes or gets an <code>services_config</code> resource or lists <code>services_config</code> in a region
...
|:-----|:--------------|:----------------|:------------|
| <CopyableCode code="get_config" /> | `SELECT` | <CopyableCode code="serviceName" /> | Gets a service configuration (version) for a managed service. |
