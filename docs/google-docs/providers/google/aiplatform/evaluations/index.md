...
| <CopyableCode code="metadata" /> | `any` | The metadata of the ModelEvaluation. For the ModelEvaluation uploaded from Managed Pipeline, metadata contains a structured value with keys of "pipeline_job_id", "evaluation_dataset_type", "evaluation_dataset_path". |
...
| <CopyableCode code="_list" /> | `EXEC` | <CopyableCode code="locationsId, modelsId, projectsId" /> | Lists ModelEvaluations in a Model. |
