...
<tr><td><b>Id</b></td><td><CopyableCode code="cloudresourcemanager.folders_iam_policies" /></td></tr>
</tbody></table>
...
| <CopyableCode code="_get_iam_policy" /> | `EXEC` | <CopyableCode code="foldersId" /> | Gets the access control policy for a folder. The returned policy may be empty if no such policy or resource exists. The `resource` field should be the folder's resource name, for example: "folders/1234". The caller must have `resourcemanager.folders.getIamPolicy` permission on the identified folder. |
