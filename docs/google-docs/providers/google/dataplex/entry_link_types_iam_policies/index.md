...
Creates, updates, deletes, gets or lists a <code>entry_link_types_iam_policies</code> resource.
...
| <CopyableCode code="projects_locations_entry_link_types_set_iam_policy" /> | `REPLACE` | <CopyableCode code="entryLinkTypesId, locationsId, projectsId" /> | Sets the access control policy on the specified resource. Replaces any existing policy.Can return NOT_FOUND, INVALID_ARGUMENT, and PERMISSION_DENIED errors. |
...
Replaces all fields in the specified <code>entry_link_types_iam_policies</code> resource.
...
WHERE 
entryLinkTypesId = '{{ entryLinkTypesId }}'
AND locationsId = '{{ locationsId }}'
AND projectsId = '{{ projectsId }}';
```
