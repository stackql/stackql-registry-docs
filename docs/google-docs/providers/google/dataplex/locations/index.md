...
| <CopyableCode code="projects_locations_lookup_entry" /> | `EXEC` | <CopyableCode code="locationsId, projectsId" /> | Looks up a single Entry by name using the permission on the source system. |
| <CopyableCode code="projects_locations_search_entries" /> | `EXEC` | <CopyableCode code="locationsId, projectsId" /> | Searches for Entries matching the given query and scope. |
