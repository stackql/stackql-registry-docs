...
| <CopyableCode code="_projects_locations_lakes_contentitems_get_iam_policy" /> | `EXEC` | <CopyableCode code="contentitemsId, lakesId, locationsId, projectsId" /> | Gets the access control policy for a contentitem resource. A NOT_FOUND error is returned if the resource does not exist. An empty policy is returned if the resource exists but does not have a policy set on it.Caller must have Google IAM dataplex.content.getIamPolicy permission on the resource. |
