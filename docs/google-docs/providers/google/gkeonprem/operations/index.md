...
| `_projects_locations_bare_metal_clusters_bare_metal_node_pools_operations_list` | `EXEC` | `bareMetalClustersId, bareMetalNodePoolsId, locationsId, projectsId` | Lists operations that match the specified filter in the request. If the server doesn't support this method, it returns `UNIMPLEMENTED`. |
| `_projects_locations_bare_metal_clusters_operations_list` | `EXEC` | `bareMetalClustersId, locationsId, projectsId` | Lists operations that match the specified filter in the request. If the server doesn't support this method, it returns `UNIMPLEMENTED`. |
