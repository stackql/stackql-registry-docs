...
| <CopyableCode code="projects_locations_bare_metal_clusters_set_iam_policy" /> | `REPLACE` | <CopyableCode code="bareMetalClustersId, locationsId, projectsId" /> | Sets the access control policy on the specified resource. Replaces any existing policy. Can return `NOT_FOUND`, `INVALID_ARGUMENT`, and `PERMISSION_DENIED` errors. |
...
Replaces all fields in the specified <code>bare_metal_clusters_iam_policies</code> resource.
