...
| <CopyableCode code="patch_per_instance_configs" /> | `EXEC` | <CopyableCode code="instanceGroupManager, project, zone" /> | Inserts or patches per-instance configurations for the managed instance group. perInstanceConfig.name serves as a key used to distinguish whether to perform insert or patch. |
