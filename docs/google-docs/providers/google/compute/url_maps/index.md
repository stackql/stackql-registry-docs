...
|:-----|:--------------|:----------------|:------------|
| `get` | `SELECT` | `project, urlMap` | Returns the specified UrlMap resource. |
...
| `_list` | `EXEC` | `project` | Retrieves the list of UrlMap resources available to the specified project. |
| `invalidate_cache` | `EXEC` | `project, urlMap` | Initiates a cache invalidation operation, invalidating the specified path, scoped to the specified UrlMap. For more information, see [Invalidating cached content](/cdn/docs/invalidating-cached-content). |
