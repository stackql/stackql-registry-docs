...
Creates, updates, deletes or gets an <code>region_target_https_proxy</code> resource or lists <code>region_target_https_proxies</code> in a region
...
|:-----|:--------------|:----------------|:------------|
| <CopyableCode code="get" /> | `SELECT` | <CopyableCode code="project, region, targetHttpsProxy" /> | Returns the specified TargetHttpsProxy resource in the specified region. |
...
Use the following StackQL query and manifest file to create a new <code>region_target_https_proxies</code> resource.
...
Updates a region_target_https_proxy only if the necessary resources are available.
...
Deletes the specified region_target_https_proxy resource.
