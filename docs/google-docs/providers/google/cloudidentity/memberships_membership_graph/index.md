...
Creates, updates, deletes or gets an <code>memberships_membership_graph</code> resource or lists <code>memberships_membership_graph</code> in a region
...
|:-----|:--------------|:----------------|:------------|
| <CopyableCode code="get_membership_graph" /> | `SELECT` | <CopyableCode code="groupsId" /> | Get a membership graph of just a member or both a member and a group. **Note:** This feature is only available to Google Workspace Enterprise Standard, Enterprise Plus, and Enterprise for Education; and Cloud Identity Premium accounts. If the account of the member is not one of these, a 403 (PERMISSION_DENIED) HTTP status code will be returned. Given a member, the response will contain all membership paths from the member. Given both a group and a member, the response will contain all membership paths between the group and the member. |
