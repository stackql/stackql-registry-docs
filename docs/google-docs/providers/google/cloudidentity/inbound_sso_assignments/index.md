...
|:-----|:--------------|:----------------|:------------|
| `inboundSsoAssignments_get` | `SELECT` | `inboundSsoAssignmentsId` | Gets an InboundSsoAssignment. |
| `inboundSsoAssignments_list` | `SELECT` |  | Lists the InboundSsoAssignments for a `Customer`. |
| `inboundSsoAssignments_create` | `INSERT` |  | Creates an InboundSsoAssignment for users and devices in a `Customer` under a given `Group` or `OrgUnit`. |
| `inboundSsoAssignments_delete` | `DELETE` | `inboundSsoAssignmentsId` | Deletes an InboundSsoAssignment. To disable SSO, Create (or Update) an assignment that has `sso_mode` == `SSO_OFF`. |
| `inboundSsoAssignments_patch` | `EXEC` | `inboundSsoAssignmentsId` | Updates an InboundSsoAssignment. The body of this request is the `inbound_sso_assignment` field and the `update_mask` is relative to that. For example: a PATCH to `/v1/inboundSsoAssignments/0abcdefg1234567&update_mask=rank` with a body of `&#123; "rank": 1 &#125;` moves that (presumably group-targeted) SSO assignment to the highest priority and shifts any other group-targeted assignments down in priority. |
