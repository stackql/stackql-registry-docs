...
| <CopyableCode code="retryLast" id="retryLast" /> | Retry the last failed run of the DAG. | `boolean` | `false` |
| <CopyableCode code="rootOnly" id="rootOnly" /> | Query parameter to filter the command output to return only root resources (resources with no predecessors). | `boolean` | `false` |
