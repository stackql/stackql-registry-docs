...
| <CopyableCode code="patch_operation" /> | `text` | field from the `properties` object |
...
| <CopyableCode code="properties" /> | `object` | The request to update subscriptions needed to be monitored by the NewRelic monitor resource. |
...
Use the following StackQL query and manifest file to create a new <code>monitored_subscriptions</code> resource.
