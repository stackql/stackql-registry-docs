...
| <CopyableCode code="change_key_vault" /> | `EXEC` | <CopyableCode code="accountName, resourceGroupName, subscriptionId, data__keyName, data__keyVaultPrivateEndpoints, data__keyVaultResourceId, data__keyVaultUri" /> | Affects existing volumes that are encrypted with Key Vault/Managed HSM, and new volumes. Supports MHSM to Key Vault, Key Vault to MHSM, MHSM to MHSM and Key Vault to Key Vault. |
| <CopyableCode code="migrate_encryption_key" /> | `EXEC` | <CopyableCode code="accountName, resourceGroupName, subscriptionId, data__privateEndpointId, data__virtualNetworkId" /> | Migrates all volumes in a VNet to a different encryption key source (Microsoft-managed key or Azure Key Vault). Operation fails if targeted volumes share encryption sibling set with volumes from another account. |
