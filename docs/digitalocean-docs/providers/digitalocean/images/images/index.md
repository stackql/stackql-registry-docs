...
| <CopyableCode code="error_message" /> | `string` | A string containing information about errors that may occur when importing<br /> a custom image. |
...
| <CopyableCode code="status" /> | `string` | A status string indicating the state of a custom image. This may be `NEW`,<br /> `available`, `pending`, `deleted`, or `retired`. |
