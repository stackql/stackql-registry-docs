...
<Tabs
    defaultValue="view"
    values={[
        { label: 'vw_client_groups', value: 'view', },
        { label: 'client_groups', value: 'resource', },
    ]
}>
<TabItem value="view">
...
| <CopyableCode code="clientGroupName" /> | `text` | field from the `properties` object |
...
system_data,
type
FROM azure.event_grid.vw_client_groups
...
Use the following StackQL query and manifest file to create a new <code>client_groups</code> resource.
...
Deletes the specified <code>client_groups</code> resource.

```sql
/*+ delete */
DELETE FROM azure.event_grid.client_groups
WHERE clientGroupName = '{{ clientGroupName }}'
