...
| <CopyableCode code="list" /> | `SELECT` |  | Gets the details of services, for a tenant, that are onboarded to Azure Active Directory Connect Health. |
...
| <CopyableCode code="_list" /> | `EXEC` |  | Gets the details of services, for a tenant, that are onboarded to Azure Active Directory Connect Health. |
| <CopyableCode code="add_alert_feedback" /> | `EXEC` | <CopyableCode code="serviceName" /> | Adds an alert feedback submitted by customer. |
| <CopyableCode code="update" /> | `EXEC` | <CopyableCode code="serviceName" /> | Updates the service properties of an onboarded service. |
