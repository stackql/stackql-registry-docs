...
| <CopyableCode code="_list" /> | `EXEC` | <CopyableCode code="scope" /> | Lists the accessReviewHistoryDefinitions available from this provider, definition instances are only available for 30 days after creation. |
| <CopyableCode code="get_by_id" /> | `EXEC` | <CopyableCode code="historyDefinitionId, scope" /> | Get access review history definition by definition Id |
