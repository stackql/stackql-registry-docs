...
Creates, updates, deletes, gets or lists a <code>access_review_instances</code> resource.
...
<Tabs
    defaultValue="view"
    values={[
        { label: 'vw_access_review_instances', value: 'view', },
        { label: 'access_review_instances', value: 'resource', },
    ]
}>
<TabItem value="view">
...
|:-----|:--------------|:----------------|:------------|
| <CopyableCode code="get_by_id" /> | `SELECT` | <CopyableCode code="id, scheduleDefinitionId, subscriptionId" /> | Get access review instances |
...
subscriptionId,
type
FROM azure.authorization.vw_access_review_instances
...
Use the following StackQL query and manifest file to create a new <code>access_review_instances</code> resource.
