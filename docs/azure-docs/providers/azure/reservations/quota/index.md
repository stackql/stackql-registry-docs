...
| <CopyableCode code="_list" /> | `EXEC` | <CopyableCode code="location, providerId, subscriptionId" /> | Gets a list of current quotas (service limits) and usage for all resources. The response from the list quota operation can be leveraged to request quota updates. |
