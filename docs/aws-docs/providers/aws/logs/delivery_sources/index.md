...
Use the following StackQL query and manifest file to create a new <code>delivery_source</code> resource, using <a ref="https://pypi.org/project/stack-deploy/" target="_blank"><code><b>stack-deploy</b></code></a>.
...
```sql
-- delivery_source.iql (required properties only)
...
```sql
-- delivery_source.iql (all properties)
INSERT INTO aws.logs.delivery_sources (
 Name,
 ResourceArn,
 LogType,
