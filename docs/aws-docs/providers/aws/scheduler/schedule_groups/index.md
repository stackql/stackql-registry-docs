...
Use the following StackQL query and manifest file to create a new <code>schedule_group</code> resource, using <a ref="https://pypi.org/project/stack-deploy/" target="_blank"><code><b>stack-deploy</b></code></a>.
...
```sql
-- schedule_group.iql (required properties only)
...
```sql
-- schedule_group.iql (all properties)
INSERT INTO aws.scheduler.schedule_groups (
 Name,
 Tags,
 region
