...
Use the following StackQL query and manifest file to create a new <code>locationf_sx_lustre</code> resource, using <a ref="https://pypi.org/project/stack-deploy/" target="_blank"><code><b>stack-deploy</b></code></a>.
...
```sql
-- locationf_sx_lustre.iql (required properties only)
...
```sql
-- locationf_sx_lustre.iql (all properties)
INSERT INTO aws.datasync.locationf_sx_lustres (
 FsxFilesystemArn,
 SecurityGroupArns,
 Subdirectory,
