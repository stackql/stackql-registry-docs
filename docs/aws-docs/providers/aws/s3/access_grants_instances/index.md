...
Use the following StackQL query and manifest file to create a new <code>access_grants_instance</code> resource, using <a ref="https://pypi.org/project/stack-deploy/" target="_blank"><code><b>stack-deploy</b></code></a>.
...
```sql
-- access_grants_instance.iql (required properties only)
...
```sql
-- access_grants_instance.iql (all properties)
INSERT INTO aws.s3.access_grants_instances (
 IdentityCenterArn,
 Tags,
 region
