...
## Fields
<table><tbody><tr><th>Name</th><th>Datatype</th><th>Description</th></tr><tr><td><CopyableCode code="backup_plan" /></td><td><code>undefined</code></td><td></td></tr>
...
## `SELECT` examples
List all <code>backup_plans</code> in a region.
```sql
SELECT
region,
backup_plan_id
...
```
Gets all properties from a <code>backup_plan</code>.
```sql
SELECT
region,
backup_plan,
