...
<tr><td><CopyableCode code="config_data" /></td><td><code>undefined</code></td><td></td></tr>
...
## `SELECT` examples
List all <code>configs</code> in a region.
```sql
SELECT
region,
arn
...
```
Gets all properties from a <code>config</code>.
```sql
SELECT
region,
name,
