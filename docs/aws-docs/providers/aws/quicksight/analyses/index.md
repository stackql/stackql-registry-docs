...
Used to retrieve a list of <code>analyses</code> in a region or to create or delete a <code>analyses</code> resource, use <code>analysis</code> to read or update an individual resource.
...
Use the following StackQL query and manifest file to create a new <code>analysis</code> resource, using <a ref="https://pypi.org/project/stack-deploy/" target="_blank"><code><b>stack-deploy</b></code></a>.
...
```sql
-- analysis.iql (required properties only)
