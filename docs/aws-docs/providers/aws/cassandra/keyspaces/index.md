...
<tr><td><CopyableCode code="replication_specification" /></td><td><code>undefined</code></td><td></td></tr>
...
## `SELECT` examples
List all <code>keyspaces</code> in a region.
```sql
SELECT
region,
keyspace_name
...
```
Gets all properties from a <code>keyspace</code>.
```sql
SELECT
region,
keyspace_name,
