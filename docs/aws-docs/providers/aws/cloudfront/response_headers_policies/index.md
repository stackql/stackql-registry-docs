...
<tr><td><CopyableCode code="response_headers_policy_config" /></td><td><code>undefined</code></td><td></td></tr>
...
## `SELECT` examples
List all <code>response_headers_policies</code> in a region.
```sql
SELECT
region,
id
...
;
```
Gets all properties from a <code>response_headers_policy</code>.
```sql
SELECT
region,
id,
