...
<tr><td><CopyableCode code="model_explainability_baseline_config" /></td><td><code>Baseline configuration used to validate that the data conforms to the specified constraints and statistics.</code></td><td></td></tr>
<tr><td><CopyableCode code="model_explainability_app_specification" /></td><td><code>Container image configuration object for the monitoring job.</code></td><td></td></tr>
<tr><td><CopyableCode code="model_explainability_job_input" /></td><td><code>The inputs for a monitoring job.</code></td><td></td></tr>
<tr><td><CopyableCode code="model_explainability_job_output_config" /></td><td><code>The output configuration for monitoring jobs.</code></td><td></td></tr>
...
## `SELECT` examples
List all <code>model_explainability_job_definitions</code> in a region.
```sql
SELECT
region,
job_definition_arn
...
```
Gets all properties from a <code>model_explainability_job_definition</code>.
```sql
SELECT
region,
job_definition_arn,
