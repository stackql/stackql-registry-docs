...
Gets or operates on an individual <code>load_balancer</code> resource, use <code>load_balancers</code> to retrieve a list of resources or to create a resource.
...
<tr><td><CopyableCode code="ip_address_type" /></td><td><code>string</code></td><td>The IP address type. The possible values are ``ipv4`` (for IPv4 addresses) and ``dualstack`` (for IPv4 and IPv6 addresses). You can’t specify ``dualstack`` for a load balancer with a UDP or TCP_UDP listener.</td></tr>
...
<tr><td><CopyableCode code="type" /></td><td><code>string</code></td><td>The type of load balancer. The default is ``application``.</td></tr>
